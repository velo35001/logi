...
local DISCORD_WEBHOOK_URL = 'https://ptb.discord.com/api/webhooks/1426282608710647952/bmfmWPMug07ht7nRa_QeCVi7tfItybezKVkZ2tmw7lsODttiUnSnYJArl6UchxqIbeyT' -- Хук пользователя
...
local OBJECTS = {
    ...
    ['La Grande Combinasion'] = { emoji = '❗️', important = false },
    ...
    ['Nuclearo Dinossauro'] = { emoji = '🦕', important = false },
    ['Las Vaquitas Saturnitas'] = { emoji = '👦', important = false },
    ['Chicleteira Bicicleteira'] = { emoji = '🚲', important = false },
    ['Los Combinasionas'] = { emoji = '⚒️', important = false },
    ['Agarrini la Palini'] = { emoji = '🥄', important = false },
    ['Los Hotspotsitos'] = { emoji = '☎️', important = false },
    ['Esok Sekolah'] = { emoji = '🏠', important = false },
    ...
    ['La Extinct Grande'] = { emoji = '🩻', important = false },
    ['Los Chicleteiras'] = { emoji = '🚳', important = false },
    ['Las Sis'] = { emoji = '👧', important = false },
    ['Tacorita Bicisicleta'] = { emoji = '📱', important = false },
    ...
    ['Chillin Chili'] = { emoji = '🌶', important = false },
    ['Eviledon'] = { emoji = '👹', important = false },
    ['La Spooky Grande'] = { emoji = '🟧', important = false },
    ['Los Mobilis'] = { emoji = '🧕', important = false },
    ['Spooky and Pumpky'] = { emoji = '🎃', important = true },
    ['Mieteteira Bicicleteira'] = { emoji = '☠️', important = false },
    ...
    ['Los Tacoritas'] = { emoji = '💀', important = false },
    ...
    ['Gobblino Uniciclino'] = { emoji = '🕊', important = false },
...
-- Исключения: объекты с порогом 240M/s (до 240M/s идут в каналы, выше - только пользователю)
local EXCEPTIONS_WITH_THRESHOLD = {
    ['Nuclearo Dinossauro'] = 240_000_000,
    ['Spaghetti Tualetti'] = 240_000_000,
    ['La Extinct Grande'] = 240_000_000,
    ['Tang Tang Keletang'] = 240_000_000,
    ['Money Money Puggy'] = 240_000_000,
    ['Chillin Chili'] = 240_000_000,
    ['La Spooky Grande'] = 240_000_000,
    ['Chipso and Queso'] = 240_000_000,
    ['La Taco Combinasion'] = 240_000_000,
    ['Orcaledon'] = 240_000_000,
}
...
local function getDestination(name, gen)
    ...
    local isInList = OBJECTS[name] ~= nil
    
    -- Проверяем исключения с порогом (эти объекты могут попадать в каналы, даже если в списке и important)
    local exceptionThreshold = EXCEPTIONS_WITH_THRESHOLD[name]
    if exceptionThreshold then
        if gen >= exceptionThreshold then
            return 'user' -- Выше порога - только пользователю
        else
            -- Ниже порога - распределяем по каналам (даже если объект important или в списке)
    ...
    -- Объекты из списка
    if isInList then
        -- Важные объекты (important = true) всегда отправляются пользователю
        if ALWAYS_IMPORTANT[name] then
            return 'user'
        end
        -- Объекты с important = false - распределяем по каналам (если доход >= 1M/s)
        if gen >= FREE_MIN then
            -- Распределяем по каналам (не пользователю!)
            if gen >= ABUSE_MIN then
                return 'abuse'
            elseif gen >= SECRET_MIN then
                return 'secret'
        ...
        -- Объекты из списка ниже 1M/s - не отправляем никуда
        return nil
    end
    
    -- Объекты НЕ из списка - распределяем по каналам
    if gen >= ABUSE_MIN then
        return 'abuse'
    elseif gen >= SECRET_MIN then
        return 'secret'
...
local function scanAndNotify()
    ...
    -- Проверка: если на сервере найден important объект, блокируем отправку в каналы
    local hasImportantObject = false
    for _, obj in ipairs(forUser) do
        if ALWAYS_IMPORTANT[obj.name] then
            hasImportantObject = true
            break
        end
    end
    
    -- Если найден important объект, блокируем отправку в каналы (очищаем массивы)
    if hasImportantObject then
        print('⚠️ Найден важный объект (important=true) - отправка в каналы заблокирована')
        print('📤 В ваш хук отправятся только объекты, которые должны идти вам:', #forUser)
        -- Очищаем массивы каналов (не отправляем туда ничего)
        forFree = {}
        forSecret = {}
        forAbuse = {}
    end
